...
class MazeSolver:
    ...
    def __init__(self, screen, rect, blocks, start_pos, end_pos):
        self.screen = screen
        self.rect = np.asarray(
            rect, dtype=int
        )  # the rect inside which the maze resides (top x, top y, width, height)
        self.blocks = blocks  # blocks should be given as (y, x) array, dtype=np.byte, 0 as a corridor and 1 as a wall
        self.start_pos = start_pos.astype(int)
        self.end_pos = end_pos.astype(int)  # the goal to seek
        ...
        self.screen_block_offset = (
            self.rect[0:2]
            + (self.rect[2:4] - self.screen_block_size * np.flip(self.block_size)) // 2
        )
        self.prev_update = 0
        self.running = True
        ...
        self.junctions = np.zeros(
            (100, 6), dtype=int
        )  # will store the junction cell, the cell before that, distance to end, and nr of directions to go
        self.junction_nr = -1
    ...
    def solve_maze(self):
        ...
        if (
            self.blocks[self.start_pos[0], self.start_pos[1]] != 0
            or self.blocks[self.end_pos[0], self.end_pos[1]] != 0
        ):
        ...
        while self.running and not np.array_equal(cell, self.end_pos):
            self.blocks[
                cell[0], cell[1]
            ] = 2  # mark as visited, prevents from turning back.
            ...
            valid_neighbors = cell_neighbors[
                (self.blocks[cell_neighbors[:, 0], cell_neighbors[:, 1]] == 0)
            ]
            ...
            if vn_count > 1:
                ...
                if self.junction_nr >= np.shape(self.junctions)[0]:
                    self.junctions = np.resize(
                        self.junctions, (self.junction_nr + 100, 6)
                    )
                ...
                self.junctions[
                    self.junction_nr, 2:4
                ] = previous  # previous location - needed when retracing the solution
                self.junctions[self.junction_nr, 4] = int(
                    np.sqrt(np.sum((self.end_pos - cell) ** 2))
                )  # distance to end_pos
                self.junctions[
                    self.junction_nr, 5
                ] = vn_count  # nr of directions (valid neighbors) available
            ...
            if vn_count == 1:
                ...
                self.junctions[
                    closest, 5
                ] -= 1  # this junction now has one less valid neighbors left
                if self.junctions[closest, 5] == 0:
                    self.junctions[
                        closest, 4
                    ] = 32767  # junction fully used - make it look like it is far away
                if np.array_equal(cell, self.junctions[closest, 0:2]):
                    ...
                    cell_neighbors = np.hstack(
                        (
                            previous + directions,
                            np.sum((self.end_pos - previous) * directions, axis=1)[
                                :, None
                            ],
                        )
                    )
        ...
        while self.running and not np.array_equal(cell, self.start_pos):
            ...
            valid_neighbors = cell_neighbors[
                (self.blocks[cell_neighbors[:, 0], cell_neighbors[:, 1]] == 2)
            ]
            if np.shape(valid_neighbors)[0] == 1:
                ...
                cell = self.junctions[: self.junction_nr + 1, 2:4][
                    (self.junctions[: self.junction_nr + 1, 0] == previous[0])
                    & (self.junctions[: self.junction_nr + 1, 1] == previous[1])
                ][0]
    ...
    def draw_cell(self, cell, previous, color):
        ...
        min_coord = (
            np.flip(np.minimum(cell, previous)) * self.screen_block_size
            + self.screen_block_offset
        ).astype(int)
        max_coord = (
            np.flip(np.maximum(cell, previous)) * self.screen_block_size
            + int(self.screen_block_size)
            + self.screen_block_offset
        ).astype(int)
        pygame.draw.rect(self.screen, color, (min_coord, max_coord - min_coord))

        if (
            self.slow_mode or pygame.time.get_ticks() > self.prev_update + 16
        ):  # increase the addition for faster but less frequent screen updates
    ...
    def toggle_info_display(self):
        ...
        if self.info_display:
            ...
            self.screen.fill(
                ...
                (x, 10, self.screen_info.get_size()[0], self.screen_info.get_size()[1]),
            )
    ...
    def plot_info(self, phase_msg):
        ...
        if self.info_display:
            ...
            x = 310
            pygame.draw.line(
                self.screen_info, self.info_color, (x - 10, y), (x - 10, y + 75)
            )
            ...
            if phase_msg[:12] == "Maze solved.":
                ...
                info_msg = (
                    f"Cell size: {self.cell_size:2d}, solution: {solution_cells} cells"
                )
                y = self.plot_info_msg(self.screen_info, x, y, info_msg)
                info_msg = (
                    f"(Visited:  {visited_cells}, junctions: {self.junctions_used})"
                )
                y = self.plot_info_msg(self.screen_info, x, y, info_msg)
            else:
    ...
    def pause(self):
        ...
        while pausing:
            event = (
                pygame.event.wait()
            )  # wait for user input, yielding to other processes
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    cell = np.flip(
                        (mouse_pos - self.screen_block_offset)
                        // (self.screen_block_size)
                    ).astype(int)
                    # make sure is within array
                    if (
                        cell[0] >= 1
                        and cell[1] >= 1
                        and cell[0] <= np.shape(self.blocks)[0] - 2
                        and cell[1] <= np.shape(self.blocks)[1] - 2
                    ):
                        ...
                        if self.blocks[cell[0], cell[1]] != 1:
                            ...
                            if is_start_pos:
                                self.draw_cell(
                                    self.start_pos, self.start_pos, self.maze_color
                                )  # previous start cell back to maze_solver color
                                self.start_pos = np.copy(cell)
                                self.draw_cell(
                                    cell, cell, self.start_color
                                )  # new start cell to start color
                            else:
                                self.draw_cell(
                                    self.end_pos, self.end_pos, self.maze_color
                                )  # previous end cell back to maze_solver color
                                self.end_pos = np.copy(cell)
                                self.draw_cell(
                                    cell, cell, self.end_color
                                )  # new end cell to end color
                            is_start_pos = not (
                                is_start_pos
                            )  # alternate between changing start cell and end cell positions
                            pygame.display.flip()
...
if __name__ == "__main__":
    ...
    maze_solver = MazeSolver(
        screen, (0, 0, 100, 100), np.ones((1, 1)), np.array([1, 1]), np.array([1, 1])
    )
    ...
    while running:
        ...
        rect = np.array(
            [0, info_display * 80, disp_size[0], disp_size[1] - info_display * 80]
        )  # the rect inside which to draw the maze.
        maze = maze_generator.Maze(
            rect[2] // (cell_size * 2) - 1, rect[3] // (cell_size * 2) - 1
        )
        ...
        maze.screen_block_offset = (
            rect[0:2]
            + (rect[2:4] - maze.screen_block_size * np.flip(maze.block_size)) // 2
        )
        ...
        if maze.running:
            maze_solver.plot_info(
                "Maze ready.  Time: {:0.2f} seconds.".format(
                    (pygame.time.get_ticks() - start_time) / 1000.0
                )
            )
            blocks_copy = np.copy(blocks)
            screen_copy = screen.copy()
            # default start and end in maze corners
            prev_start_pos = np.array([-1, -1], dtype=int)
            prev_end_pos = np.array([-1, -1], dtype=int)
            prev_msg = maze_solver.last_message
            prev_mode = maze_solver.slow_mode
            prev_info = maze_solver.info_display
            start_pos = (
                np.asarray(np.shape(blocks), dtype=int) - 2
            )  # bottom right corner
            end_pos = np.array([1, 1], dtype=int)
            ...
            while (
                running
                and maze_solver.cell_size == prev_cell_size
                and not (
                    np.array_equal(maze_solver.start_pos, prev_start_pos)
                    and np.array_equal(maze_solver.end_pos, prev_end_pos)
                )
            ):
                ...
                maze_solver.draw_cell(
                    maze_solver.end_pos, maze_solver.end_pos, maze_solver.end_color
                )
                start_time = pygame.time.get_ticks()
                ...
                if maze_solver.running:
                    maze_solver.plot_info(
                        "Maze solved. Time: {:0.2f} seconds.".format(
                            (pygame.time.get_ticks() - start_time) / 1000.0
                        )
                    )
