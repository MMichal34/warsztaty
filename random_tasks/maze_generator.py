...
class Maze:
    ...
    def __init__(self, size_x, size_y):
        ...
        self.wall_size = np.array([size_y, size_x], dtype=int)
        ...
        self.block_size = np.array([size_y * 2 + 1, size_x * 2 + 1], dtype=int)
    ...
    def gen_maze_walls(self, corridor_len=999):
        ...
        cell = np.array([random.randrange(2, self.wall_size[0]), random.randrange(2, self.wall_size[1])], dtype=int)
        ...
        while np.size(cell) > 0 and self.running:
            ...
            if np.size(valid_neighbors) > 0:
                ...
                if round_nr - corridor_start < corridor_len:
                    ...
                    cell = np.array([neighbor[0], neighbor[1]], dtype=int)
                else:
